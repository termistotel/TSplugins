...
PUT=$HOME"/TeamSpeak3-Client-linux_amd64/plugins/lua_plugin/Anna/RadioBabura/emisije/EmisijePodcastovi/Latecraft"
OPIS="Now airing: Late Night Lovecraft - Bringing a taste of The Great Things From Far Beyond to your homes every night at 23:15 on RadioBabura."
...
case "$1" in
	play)
		...
		echo -e "\n"$OPIS"\nTrenutno je epizoda: [URL="$TRENUTNI"]"$TRENUTNI > $PUT1/currentSong		#dodavanje da se opis može vidjet sa "trenutna?"

		youtube-dl --no-progress -f mp4 $TRENUTNI -o - | ffmpeg -i - -vn -f s16le - | playraw			#skini video s linka, konvertiraj ga u sirovi format i pusti
